...
export default function SpotDetails() {
  ...
  const averageRating =
    spotReviews.length >= 1
      ? spotReviews.reduce((acc, ele) => acc + ele.stars, 0) /
        spotReviews.length
      : "New!";
  ...
  const serviceFee = (singleSpot.price * stayDuration * 0.1).toFixed(0);
  const unavailable = bookings?.map((ele) => ({
    start: new Date(ele.startDate),
    end: new Date(ele.endDate),
  }));
  ...
  const handleCheckin = (date) => {
    ...
    if (date >= new Date(checkout))
      setCheckout(new Date(Date.parse(date) + 86400000));
  };
    ...
    <div className="spot-details-page">
      <div className="spot-details-90">
        <div className="spot-details">
          ...
          <h5>
            {reviewLogic()} ● {singleSpot.city}, {singleSpot.state},{" "}
            {singleSpot.country}
          </h5>
            ...
            <div className="detail-images-left">
              <img
                src={
                  ...
                  singleSpot?.SpotImages[0]?.url
                }
                alt="main"
                key={`detail-image-1`}
                className={`detail-image detail-image-0`}
              ></img>
            </div>
              ...
              {singleSpot.SpotImages.map((ele, i) =>
                ...
                i > 0 && i < 5 ? (
                  <img
                    ...
                    alt="main"
                    key={`detail-image-${i}`}
                    className={`detail-image detail-image-${i}`}
                  ></img>
                ) : null
              )}
            </div>
          </div>
          <h1 className="hosted-by">
              ...
              <div className="spot-accolades">
                ...
                <div className="dedicated-workspace spot-accolade">
                  ...
                  <div className="accolade-text">
                    ...
                    <div className="accolade-grey-text">
            ...
            <div className="price-review-reserve">
              <div className="reserve-text">
                <span className="reserve-left">
                  <span className="reserve-left-price">
                    ${singleSpot.price}{" "}
                  </span>{" "}
                  <span className="reserve-night">night</span>
                </span>
                <span className="reserve-right">{reviewLogic()}</span>
              </div>
              ...
              <table className="reserve-breakdown">
                <tbody>
                  <tr>
                    <td className="reserve-item">
                      {singleSpot.price} x {stayDuration} nights
                    </td>
              ...
              <div className="reserve-total-before-taxes">
                <span>Total Before Taxes</span>{" "}
                <span className="reserve-total-price">${total}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="reviews-printed">
          ...
          {spotReviews && spotReviews.length ? (
            spotReviews
                ...
                <div className="review-container" key={`review-container${i}`}>
                  ...
                  >
                    ...
                    <div className="review-text">
                      <div className="username-title" key={"username-iter" + i}>
                        {rev.User.firstName}
                      </div>
                  ...
                  <span className="review-body" key={"reviewbody" + i}>
                    {rev.review}
                  </span>
                  {
          ...
          ) : user && singleSpot.Owner.id !== user.id ? (
            <p>Be the first to post a review!</p>
          ) : null}
        </div>
        <hr></hr>
        <h4>Where you'll be</h4>
        <h5 className="grey where-youll-be">
