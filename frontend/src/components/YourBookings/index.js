...
export default function YourBookings() {
    ...
    return (
        ...
        <div className='your-bookings-content'>
            ...
            <hr></hr>
            {upcomingBookings.length ? upcomingBookings.map((book, i) => <Bookings i={i} book={book} />)  : (<h4>Go book a trip!</h4>)}
            <h3>Past Reservations</h3>
            <hr></hr>
            {pastBookings.length ? pastBookings.map((book, i) => <Bookings i={i} book={book} />) : (<h4>Nothing here yet!</h4>)}
        </div>
        </div>
    )
}
